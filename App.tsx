...
function App() {
  // Route check: Only show Admin if URL contains ?portal=admin
  ...
  useEffect(() => {
    // Check for Admin Portal Request
    const params = new URLSearchParams(window.location.search);
    if (params.get('portal') === 'admin') {
       setIsAdminMode(true);
       setLoading(false); // Skip auth loading for admin view (it handles its own secure view in real app)
       return;
    }
      ...
      return (
         <ErrorBoundary>
            <AdminDashboard onExit={() => {
                // Remove query param to exit admin mode
  ...
  const SettingsView = () => (
    <div className="max-w-xl mx-auto pt-4 pb-24 animate-fade-in">
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-xl text-left border border-stone-100 mb-8">
        ...
        {/* Separated Admin Link */}
        <div className="border-t border-stone-100 pt-8 text-center">
            <a 
               href="?portal=admin"
               className="inline-flex items-center gap-2 text-stone-300 hover:text-iroko-dark text-xs font-bold uppercase tracking-widest transition-colors"
            >
               <ExternalLink size={12} /> Access Admin Portal
            </a>
        </div>
      </div>
    </div>
